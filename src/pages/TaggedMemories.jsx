...
import { Heart, Check, X, Tag, Bookmark } from 'lucide-react';
      ...
      <KeepsakeCard>
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="min-w-0">
            <p className="text-section-title truncate">{share.memory_title || 'Untitled memory'}</p>
            <p className="text-caption mt-0.5">
              Shared by {share.from_user_name || 'someone'}
            </p>
          </div>
          {share.shared_at && (
            <span className="text-caption shrink-0">
              {format(new Date(share.shared_at), 'MMM d')}
            </span>
          )}
        </div>
        ...
        {share.status === 'pending' && (
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="outline" onClick={() => handleTag(share)} className="rounded-xl gap-1 flex-1">
              <Bookmark className="w-3.5 h-3.5" /> Tag
            </Button>
            <Button size="sm" variant="ghost" onClick={() => handleReject(share)} className="rounded-xl text-destructive px-3">
        ...
        {share.status === 'accepted' && share.added_thoughts && (
          <div className="mt-2 p-3 bg-secondary/50 rounded-xl">
            <p className="text-caption italic">Your response</p>
            <p className="text-body mt-1">&ldquo;{share.added_thoughts}&rdquo;</p>
            {share.added_emotion && (
        ...
        {share.status === 'tagged' && (
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-caption">Saved to tagged folder</span>
          </div>
        )}
      </KeepsakeCard>
    </motion.div>
      ...
      <Dialog open={!!showRespond} onOpenChange={() => setShowRespond(null)}>
        <DialogContent className="rounded-2xl">
          ...
          <div className="space-y-4 pt-2">
            ...
            <Button onClick={handleSubmitResponse} className="w-full rounded-xl gap-2">
              <Check className="w-4 h-4" /> Add to my timeline
            </Button>
          </div>
        </DialogContent>
      </Dialog>
