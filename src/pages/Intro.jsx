...
import { ChevronRight, Play, Pause, Heart, Sparkles, Clock, Shield, Users, ArrowRight } from 'lucide-react';
...
const scenes = [
  ...
  },
  {
    ...
    subtitle: 'Your words travel across the years. A note written today can reach your child on their 18th birthday, or your partner on your anniversary.',
    icon: Clock,
...
export default function Intro() {
  ...
  const [hasSeenIntro] = useState(() => {
    try { return localStorage.getItem('hasSeenIntro') === 'true'; }
    catch { return false; }
  });
    ...
    <div className="fixed inset-0 bg-background overflow-hidden">
      <AnimatePresence mode="wait">
        ...
        >
          ...
          />
          <div className={`absolute inset-0 bg-gradient-to-b ${scene.color} to-background/95`} />
      ...
      <button
        type="button"
        onClick={() => { markSeen(); navigate('/'); }}
        className="absolute top-6 right-6 z-20 text-caption text-white/80 hover:text-white font-medium transition-colors"
      >
        Skip
      </button>
        ...
        <div className="px-6 pb-10">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="text-body sm:text-lg text-white/95 leading-relaxed max-w-md drop-shadow-[0_2px_8px_rgba(0,0,0,0.6)]">
                {scene.subtitle}
              </p>
              ...
              <div className="flex items-center gap-3 pt-2">
                <button
                  type="button"
                  onClick={togglePlay}
                  className="w-10 h-10 rounded-full bg-white/15 backdrop-blur-sm flex items-center justify-center hover:bg-white/25 transition-colors border border-white/10"
                ...
                <div className="flex gap-1.5" role="tablist" aria-label="Intro scenes">
                  {scenes.map((_, i) => (
                    <div
                      key={i}
              ...
              {!isLast && videoEnded && (
                <p className="text-caption text-white/70 text-center">Tap Next to continue the story</p>
              )}
            </motion.div>
          </AnimatePresence>
        </div>
